...
<body>
    ...
    document.querySelectorAll('.sidebar-list-item').forEach(item => {
        item.addEventListener('click', (event) => {
            ...
            // Define the mapping of menu items to their respective URLs
            const menuItems = {
                'dash-nav': 'manage/myaccount',
                'production-nav': 'manage/myorders',
                'orders-nav': 'manage/myreviews',
                'workers-nav': 'manage/mywishlist'
            };

            // Redirect to the corresponding URL if it exists
            if (menuItems.hasOwnProperty(id)) {
                const url = 'http://localhost/wcf/' + menuItems[id];
                window.location.href = url;
            } else {
                console.error('No URL found for menu item:', id);
            }
        });
    });
</script>
